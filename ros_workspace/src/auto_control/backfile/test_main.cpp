...
int main(int argc,char **argv){
    ...
    msg_L.X_Axis=InitL_PosX;msg_L.Y_Axis=InitL_PosY;msg_L.Z_Angle=InitL_PosAngle;
    msg_R.X_Axis=InitR_PosX;msg_R.Y_Axis=InitR_PosY;msg_R.Z_Angle=InitR_PosAngle;
    ...
    const float target_height=2.0;
    const float theta_miu=52.0; //接触面为高摩擦区对应的弧度角

    // msg_L.Z_Angle=-theta_miu/2;
    ...
    while (ros::ok())
    {
        ...
        */
        ...
        case moveL_up:
            ...
            // if((msg_L.Y_Axis-InitL_PosY)>((theta_miu/2)*deg2rad*Radius))
            //     flag_=turnL_anticlock;
            if((msg_L.Y_Axis-InitL_PosY)>((theta_miu/2)*deg2rad*Radius)+(theta_miu/2/deta_theta)*Modify)
                flag_=turnL_anticlock;
            ROS_INFO("moveL_up");
            break;
