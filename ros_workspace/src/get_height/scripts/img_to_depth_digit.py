...
class ImageToDepth(object):
    ...
    def __init__(self,folder):
        ...
        self.camera_parameter = pickle.load(open("/home/jhz/catkin_ws/src/get_height/digiteye/cam/cam2_calib.pkl", 'rb'))
        self.lookup_table = pickle.load(open(folder + '/Lookuptable.pkl', 'rb'))
        self.border = self.lookup_table[9]
        frame0 = cv2.imread(folder +'/0.jpg')      #./0.jpg是无按压情况下的rgb图像，应该是用于与按压情况下的rgb求差
    ...
    def _init_frame(self, frame0):
        sigma = 50
        f0 = cv2.GaussianBlur(frame0, (99,99), sigma)   #高斯滤波（线性平滑滤波）用于消除高斯噪声，图像的减噪
    ...
    def _fast_poisson(self, gx, gy):
        ...
        img_direct = np.zeros((ydim, xdim))              #此时处理的图片尺寸维度对应的是标定digit过程中，裁剪的第一帧图像的尺寸
    ...
    def convert(self, img):
        ...
        d_ptcd[:, 2] = hm.flatten()*3    #原本的，*20应该是为了在点云图中更加清晰的显示数据
